...
cache Png_Create(cache data, int* size, int width, int height, int paloffset, int offsetx = 0, int offsety = 0, bool sky = false)
{
    ...
    for(int x = 0;x < palsize;x++)
    {
       ...
       if(palette[x].red==248 && palette[x].green==0 && palette[x].blue==248)
          gettrans = x;
       else if(palette[x].red==248 && palette[x].green==0 && palette[x].blue==240)
          gettrans = x;
    }
    fclose(fpal);
...
void Create_Walls(int file, int startoff,int count,int paloffset, int x0, int x1, int y0, int y1)
{
    ...
    if(width <= 8){width2 = 8;}
    if(width <= 4){width2 = 4;}
    if(width <= 0){width2 = 64;}
    width += (width2-width);
    int height = count/width;

    //printf("width %d height %d\n",width,height);
    
    sprintf(string,"Datos/tex%2.2d.bin",file);
...
void Create_Sprites(int file, int startoff,int count,int paloffset, int x0, int x1, int y0, int y1)
{
    ...
    int endoffset = startoff+count-2;
...
static void initflags(int paramInt)
{
    ...
    while (i1 < paramInt)
    {
      ...
      int tmp53_51 = i2;
      //printf("Paleta %d\n",tmp53_51);//pal
      short tmp53_50 = Pals[tmp53_51];
      Pals[tmp53_51] = ((short)(tmp53_50 | 0x4000));
      ...
      int tmp87_85 = i2;
      //printf("Texture %d\n",tmp87_85);//tex
      short tmp87_84 = Tex[tmp87_85];
      Tex[tmp87_85] = ((short)(tmp87_84 | 0x4000));
      i1++;
    }
}
...
static void inittex()
{
    ...
    for (i7 = 0; i7 < 1024; i7++)
    {
      ...
      if (i8 != 0)
      {
        if ((((i8 = i10 = (Tex[i7] & 0x1FFF) + 1) - 1 & i8) == 0 ? 1 : 0) != 0)
        {
          //printf("i6 %d i10 %d\n",i6,i10);//Tex
          int tmp141_139 = i7;
          Tex[tmp141_139] = ((short)(Tex[tmp141_139] | 0x2000));
        }
        else
        {
          i4 += i10;
    ...
    {
      int i2 = (Tex[i1] & 0x4000) != 0 ? 1 : 0;
      i5 = (Tex[i1] & 0x8000) != 0 ? 1 : 0;
      int i13 = (Tex[i1] & 0x1FFF) + 1;
      int i14 = (Tex[i1] & 0x2000) != 0 ? 1 : 0;
...
void ExtraerSKYS(int idx, int num)
{
    ...
    PrintfPorcentaje(1,1,true, 10+num,"Extrayendo SKY%02d......",num);
    ...
    int pngsize = 0;
    cache pngout = Png_Create(input, &pngsize, 128, 128, idx-1, 0, 0, true);
...
void ShowInfo()
{
    ...
    setcolor(0x07);printf("#############\n"); 
    printf("     #       DOOM II RPG EXTRACTOR     #\n");
    printf("     # CREADO POR ERICK VASQUEZ GARCIA #\n");
    printf("     #       ES PARA DOOM II RPG       #\n");
    printf("     #         VERSION 5.1.80          #\n");
    printf("     #          MODO DE USO:           #\n");
...
void ExtraerDatos()
{
    hz = OpenZip(("doom_2_rpg_v_5.1.80.jar"),0);
    if(hz == NULL){error("No encuentra doom_2_rpg_v_5.1.80.jar");}
    SetUnzipBaseDir(hz,("Datos"));
    ZIPENTRY ze; GetZipItem(hz,-1,&ze); int numitems=ze.index;
    
    int i;
    FindZipItem(hz,("newMappings.bin"),true,&i,&ze);
    UnzipItem(hz,i,ze.name);
    FindZipItem(hz,("newPalettes.bin"),true,&i,&ze);
    UnzipItem(hz,i,ze.name);
    FindZipItem(hz,("newPalettes.bin"),true,&i,&ze);
    UnzipItem(hz,i,ze.name);
    FindZipItem(hz,("tables.bin"),true,&i,&ze);
    UnzipItem(hz,i,ze.name);
    
    for(int j = 0; j <= 13; j++)
    {
        sprintf(string,"tex%2.2d.bin",j);
...
int main(int argc, char *argv[])
{
    ...
    for(i = 0; i < 512; i++)
    {
      PrintfPorcentaje(i,512-1,true, 9,"Extrayendo Sprites Y Texturas......");
      if(Unk01[i] != -1)
      {
       if(Unk01[i+1] == -1) continue;
       //printf("\ni %4.4X\n",i);
